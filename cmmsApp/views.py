...
def request_demo_view(request):
    ...
    subject = "New Ground Support Equipment (GSE) Request"
    text_body = (
        "A new Ground Support Equipment (GSE) request was submitted.\n\n"
        f"Submitted: {ts}\n"
    ...
    html_body = f"""
        <h2 style="margin:0 0 8px">New Ground Support Equipment (GSE) Request</h2>
...
def contact_section(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        # Email body
        subject = "New contact submission for Ground Support Equipment (GSE)"
        text_body = "\n".join(
            [
                "New contact submission for Ground Support Equipment (GSE):",
...
def contact_block_submit(request):
    ...
    # --- Email notification ---
    subject = f"[Ground Support Equipment (GSE)] Consulting request: {name}"
 
    text_body = "\n".join([
        "A new consulting request was submitted for Ground Support Equipment (GSE):",
        f"Name: {name}",
        f"Email: {email}",
